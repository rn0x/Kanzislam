 ...
 * @param {Array} Pages - مصفوفة من الصفحات التي يجب إضافتها إلى خريطة الموقع.
 * @returns {Promise<Object>} - معلومات حول خرائط الموقع التي تم إنشاؤها.
 */
export default async function sitemapUtlis(Pages) {
  try {
    const __dirname = path.resolve();
    const sitemapDir = path.join(__dirname, 'src/public/sitemap');
    const sitemapSizeLimit = process.env.SITEMAP_LIMIT || 5000;
    ...
    const sitemapIndexContent = generateSitemapIndexContent(numSitemaps, process.env.WEBSITE_DOMAIN);
